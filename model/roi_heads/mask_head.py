...
__all__ = ["BaseMaskRCNNHead", "MaskRCNNConvUpsampleHead"]
...
def mask_rcnn_loss(pred_mask_logits: torch.Tensor, instances: List[Instances], vis_period: int = 0):
    ...
    gt_masks = torch.cat(gt_masks, dim=0).float()
...
class MaskRCNNConvUpsampleHead(BaseMaskRCNNHead, nn.Sequential):
    ...
    def __init__(self, input_shape, *, num_classes, conv_dims, conv_norm=None, **kwargs):
        """
        NOTE: this interface is experimental.

        Args:
            input_shape (dict or int): shape of the input feature to this module (HxWxC). If int, then only the channel dimension is set.
        ...
        self.deconv = nn.ConvTranspose2d(cur_channels, conv_dims[-1], kernel_size=2, stride=2, padding=0)
        self.add_module("deconv_relu", nn.ReLU())
        cur_channels = conv_dims[-1]

        self.predictor = nn.Conv2d(cur_channels, num_classes, kernel_size=1, stride=1, padding=0)
