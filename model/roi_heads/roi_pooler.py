...
from .utils import assert_fx_safe, is_fx_tracing
...
def convert_boxes_to_pooler_format(box_lists: List[Boxes]):
    ...
    sizes = torch.tensor([x.__len__() for x in box_lists])
...
class ROIPooler(nn.Module):
    ...
    def __init__(
        ...
        if pooler_type == "ROIAlign":
            ...
            raise NotImplementedError("ROIAlignRotated is not supported")
            self.level_poolers = nn.ModuleList(
